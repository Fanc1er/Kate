...
func (s *TriageService) guard() *repository.Guard {
...
func (s *TriageService) ListFindings(p FindingListParams) ([]models.Finding, int64, error) {
	q := s.guard().Scoped(&models.Finding{})
	if p.Status != "" {
...
func (s *TriageService) UpdateFindingStatus(id int64, status string, userID int64, username, ip, ua string) (*models.Finding, error) {
	...
	if status == "closed" {
		now := time.Now()
		s.DB.Model(&models.Alert{}).Where("finding_id = ?", id).
			Updates(map[string]any{"status": "resolved", "resolved_at": now})
		s.DB.Model(&models.Vulnerability{}).Where("finding_id = ?", id).
			Updates(map[string]any{"status": "closed", "closed_at": now})
	}
	if s.Audit != nil {
		s.Audit.Write(userID, username, "finding.update", "finding", fmt.Sprint(id), f.Status, status, ip, ua)
	}
	return &f, nil
}

// GetFindingDetail finding 详情 + 证据链。
func (s *TriageService) GetFindingDetail(id int64) (map[string]any, error) {
	var f models.Finding
	if err := s.DB.Where("id = ?", id).First(&f).Error; err != nil {
		return nil, err
	}
	...
	if len(evIDs) > 0 {
		s.DB.Where("id IN ?", evIDs).Find(&evs)
	}
	...
	if f.Type == "sensitive_info" {
		...
		s.DB.Where("task_id = ? AND url = ?", f.TaskID, f.URL).
...
// ListEvents 事件列表。
func (s *TriageService) ListEvents(status, eventType string, page, pageSize int) ([]models.Event, int64, error) {
	q := s.guard().Scoped(&models.Event{})
	if status != "" {
...
func (s *TriageService) UpdateEventStatus(id int64, status string, userID int64, username, ip, ua string) error {
	...
	if s.Audit != nil {
		s.Audit.Write(userID, username, "event.update", "event", fmt.Sprint(id), ev.Status, status, ip, ua)
	}
	return nil
}

// GetEventDetail 事件详情（主记录 + 关联 findings + 关联工单）。
func (s *TriageService) GetEventDetail(id int64) (map[string]any, error) {
	var ev models.Event
	if err := s.DB.Where("id = ?", id).First(&ev).Error; err != nil {
		return nil, err
	}
	...
	if len(fIDs) > 0 {
		...
		s.DB.Where("id IN ?", fIDs).Find(&findings)
		detail["findings"] = findings
	}
	ticketIDs, err := s.EventTicketIDs(id)
	if err == nil && len(ticketIDs) > 0 {
		var tickets []models.Ticket
		s.DB.Where("id IN ?", ticketIDs).Find(&tickets)
...
func (s *TriageService) BatchUpdateEventStatus(ids []int64, status string, userID int64, username, ip, ua string) (map[string]any, error) {
	var success, failed []int64
	for _, id := range ids {
		if err := s.UpdateEventStatus(id, status, userID, username, ip, ua); err != nil {
...
// ListAlerts 告警列表。
func (s *TriageService) ListAlerts(status, alertType string, page, pageSize int) ([]models.Alert, int64, error) {
	q := s.guard().Scoped(&models.Alert{})
	if status != "" {
		q = q.Where("status = ?", status)
	}
	if alertType != "" {
...
func (s *TriageService) GetAlertDetail(id int64) (map[string]any, error) {
	var a models.Alert
	...
	if a.FindingID > 0 {
		var f models.Finding
		if err := s.DB.Where("id = ?", a.FindingID).First(&f).Error; err == nil {
			detail["finding"] = f
		}
	}
	return detail, nil
...
// ResolveAlert 关闭告警。
func (s *TriageService) ResolveAlert(id int64, userID int64, username, ip, ua string) error {
	var a models.Alert
	if err := s.DB.Where("id = ?", id).First(&a).Error; err != nil {
		return err
	}
	...
	if s.Audit != nil {
		s.Audit.Write(userID, username, "alert.resolve", "alert", fmt.Sprint(id), a.Status, "resolved", ip, ua)
	}
	return nil
}

// BatchResolveAlert 批量关闭告警。
func (s *TriageService) BatchResolveAlert(ids []int64, userID int64, username, ip, ua string) (map[string]any, error) {
	var success, failed []int64
	for _, id := range ids {
		if err := s.ResolveAlert(id, userID, username, ip, ua); err != nil {
			failed = append(failed, id)
		} else {
			success = append(success, id)
		}
...
// ListVulns 漏洞列表。
func (s *TriageService) ListVulns(status, severity string, assetID int64, page, pageSize int) ([]models.Vulnerability, int64, error) {
	q := s.guard().Scoped(&models.Vulnerability{})
	if status != "" {
		q = q.Where("status = ?", status)
	}
	if severity != "" {
...
func (s *TriageService) GetVulnDetail(id int64) (map[string]any, error) {
...
func (s *TriageService) GetVulnEvidence(id int64) ([]map[string]any, error) {
	var v models.Vulnerability
	if err := s.DB.Where("id = ?", id).First(&v).Error; err != nil {
	...
	for _, ev := range evs {
		files, err := s.Evidence.Files(ev.ID)
		if err != nil {
			continue
		}
...
func (s *TriageService) TriageOverview() (map[string]any, error) {
	type cnt struct {
		Status string
		Cnt    int64
	}
	...
	s.DB.Model(&models.Finding{}).Select("status, COUNT(*) AS cnt").Group("status").Scan(&fRows)
	s.DB.Model(&models.Event{}).Select("status, COUNT(*) AS cnt").Group("status").Scan(&eRows)
	s.DB.Model(&models.Alert{}).Select("status, COUNT(*) AS cnt").Group("status").Scan(&aRows)
	s.DB.Model(&models.Vulnerability{}).Select("status, COUNT(*) AS cnt").Group("status").Scan(&vRows)
	for _, r := range fRows {
	...
	// 今日新增。
	var todayNew int64
	s.DB.Model(&models.Alert{}).Where("created_at >= ?", time.Now().Truncate(24*time.Hour)).Count(&todayNew)
