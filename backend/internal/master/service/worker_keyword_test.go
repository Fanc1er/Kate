...
func TestPullTaskKeywordRules(t *testing.T) {
	...
	policy := models.ScanPolicy{Name: "p", Concurrency: 4, Timeout: 60, ScanDepth: 2, ConcurrencyLimit: 4, SameOrigin: true, CrawlSubpages: true}
	if err := gdb.Create(&policy).Error; err != nil {
		t.Fatalf("create policy: %v", err)
	}
	task := models.ScanTask{AssetID: asset.ID, PolicyID: policy.ID, Status: models.StatusPending}
	if err := gdb.Create(&task).Error; err != nil {
		t.Fatalf("create task: %v", err)
	}
	// 组织自定义 keyword 规则 + 全局 keyword 规则。
	rules := []models.RuleDefinition{
		{Kind: "keyword", Name: "竞品词", FRegex: `超级竞品`, Sensitive: false, Loaded: true},
		{Kind: "keyword", Name: "违规词", FRegex: `违规内容`, SRegex: `公司活动`, Sensitive: true, Loaded: true},
		{Kind: "keyword", Name: "未加载", FRegex: `x`, Loaded: false},
		{Kind: "sensitive", Name: "非关键词", FRegex: `身份证`, Loaded: true},
	}
	for i := range rules {
