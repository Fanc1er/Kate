...
CInsight 是基于 Master-Worker 架构的单租户私有化安全监测平台。Master 负责 API、任务调度、持久化与证据管理，Worker 负责执行 10 大检测引擎并回传结果。平台以"全量证据链"为核心取证理念，通过 SHA-256 签名保证证据不可篡改，通过 RBAC 控制功能访问，并通过离线授权文件控制系统可用性与资源配额。
...
| AuthMiddleware | JWT 校验 + 解析 user_id/role | `Authorization: Bearer {jwt}` |
| RBACMiddleware | 按角色校验写操作权限 | `RequireRoles(role...)`, `RequireWrite()` |
| LicenseMiddleware | 校验离线授权状态，未授权/过期时拒绝业务 API | `CheckLicense(ctx)` |
...
| CronScheduler | 按 scan_plans.cron_expr 定时生成 scan_tasks、按 report_templates.period/cron_expr 定时生成 reports、重保/护网激活期间按日生成 daily_war_reports（计划/模板绑定时区计算；paused/授权失效跳过触发，授权恢复后继续；计划 time_window 窗口外跳过触发） | `GenerateTasks()`, `Tick()` |
...
- **系统级邮件发送**：forgot-password 验证码、reset-password 确认、成员邀请邮件由系统级 SMTP（`CINSIGHT_SMTP_*`）发送，与 `notify_channels` 的 SMTP 渠道解耦——登录前与未登录场景无用户上下文，只能走系统 SMTP。验证码 5 分钟有效、一次性使用（服务端存储 hash + 失效时间，使用后即删）。
...
- 内容安全引擎的 AI 文本分类走 `AIAdapter`，支持可配置 `base_url / model / api_key`（环境变量 `CINSIGHT_LLM_BASE_URL`、`CINSIGHT_LLM_MODEL`、`CINSIGHT_LLM_API_KEY` 注入），api_key 由管理员自行配置（K8s Secret），禁止硬编码。
...
- 鉴权分层：
  - **JWT 用户鉴权**（前端会话）：`Authorization: Bearer {access_token}`，经 AuthMiddleware + RBACMiddleware；短时效 access token（15min），refresh token（7d）经 `POST /api/v1/auth/refresh` 换发，登出/改密/重置后 jti 黑名单立即失效。
  - **API Token 鉴权**（开放集成）：`Authorization: Bearer {api_token}`，scopes 细粒度控制，独立于 JWT。
...
角色：**a**=admin（管理员）、**u**=user（普通用户）。`✓`=允许，`-`=禁止。所有写操作 API 必经 RBACMiddleware 校验，`user` 对配置/管理类写操作返回 403。
...
| 授权管理（状态查询/机器码/导入） | 无门禁（授权前可访问，导入靠授权文件签名校验） | |
| 平台统计/Worker 总览：读 | ✓ | ✗ |
| 扫描场景：读 | ✓ | ✗ |
| 扫描场景：写（CRUD/激活/停用） | ✓ | ✗ |
| 告警升级规则：读 | ✓ | ✗ |
...
| 值守：写（交接班） | ✓ | ✓ |
| 每日战报：读（列表/下载） | ✓ | ✓ |
| 认证：login/forgot-password/reset-password | 公开 | 公开 |
| 认证：refresh/change-password/logout/delete-account/me/data-export | 登录用户（依赖 JWT，无需角色判断） | |
| WebSocket /ws/events | 登录用户（JWT 鉴权） | |
| Worker 内部 /worker/*（register/tasks/evidence/heartbeat） | Worker 凭证鉴权（Bootstrap 一次性 / client_id+secret），不经 RBAC | |

**权限码清单**（`src/config/permissions.ts`，与上表一致，菜单/路由/按钮三级共用同一数据源）：`asset:read/export`、`asset:write`、`asset:batch-delete`、`wechat:write`、`policy:write`、`plan:write`、`task:write`、`task:delete`、`event:write`、`noise:write`、`alert:write`、`vuln:write`、`ticket:write`、`evidence:upload`、`report:delete`、`report-template:write`、`member:write`、`worker:write`、`channel:write`、`route:write`、`rules:write`、`intel-sub:write`、`whitelist:write`、`token:write`、`webhook:write`、`scenario:write`（场景 CRUD/激活/停用）、`escalation:write`（告警升级规则 CRUD）、`watch:write`（交接班）、`platform:read`（仅 admin）。`user` 无任何配置类 `*:write` 权限码，配置/管理写按钮直接不渲染。
...
| 认证 | GET | /api/v1/auth/me | 登录用户 | 当前用户信息（仅依赖 JWT，前端渲染导航/按钮权限） |
| 授权 | GET | /api/v1/license/machine-code | 公开（无门禁） | 获取机器码（授权签发前向厂商提供） |
| 授权 | GET | /api/v1/license/status | 公开（无门禁） | 授权状态（授权状态/到期时间/配额上限，授权前可查） |
| 授权 | POST | /api/v1/license/import | 公开（无门禁，靠授权文件签名校验） | 导入授权文件（校验签名/机器匹配/有效期，更新授权状态） |
| 平台 | GET | /api/v1/platform/stats | admin | 平台统计 |
| 平台 | GET | /api/v1/platform/workers | admin | 平台 Worker 总览 |
...
| 资产 | GET/POST | /api/v1/assets | admin/user | 资产列表/创建 |
| 资产 | PUT/DELETE | /api/v1/assets/:id | admin/user | 资产编辑/删除 |
...
| 资产 | POST | /api/v1/assets/:id/dual-ua | admin/user | 双 UA 对比（正常 UA + 蜘蛛 UA 抓取，返回差异列表） |
| 资产 | GET | /api/v1/assets/:id/profile | 全部角色 | 资产画像（技术栈指纹/ICP 备案/子域名/SSL 证书倒计时/端口服务快照） |
| 资产 | POST | /api/v1/assets/batch-scan | admin/user | 批量资产加入扫描（ids + policy_id） |
| 资产 | POST | /api/v1/assets/batch-delete | admin | 批量删除资产 |
| 资产 | POST | /api/v1/assets/batch-group | admin/user | 批量改分组（ids + group_name） |
| 资产 | POST | /api/v1/assets/batch-import | admin/user | URL 列表/CSV 批量导入 |
| 资产 | GET | /api/v1/assets/import-template | 全部角色 | 导入模板下载（URL/CSV 模板文件） |
| 资产 | GET | /api/v1/assets/export | 全部角色 | 当前筛选结果 CSV 导出 |
| 资产 | GET/POST | /api/v1/wechat-assets | admin/user | 微信公众号资产 |
| 资产 | PUT/DELETE | /api/v1/wechat-assets/:id | admin/user | 公众号资产编辑/删除 |
| 策略 | GET/POST | /api/v1/policies | admin（GET 允许 user，任务创建选模板） | 策略模板列表/创建 |
| 策略 | PUT/DELETE | /api/v1/policies/:id | admin | 策略模板编辑/删除 |
| 策略 | POST | /api/v1/policies/:id/copy | admin | 复制模板（深拷贝引擎开关，新名可改） |
| 策略 | POST | /api/v1/policies/batch-delete | admin | 批量删除策略模板 |
| 计划 | GET/POST | /api/v1/scan-plans | admin | Cron 定时计划 |
| 计划 | PUT/DELETE | /api/v1/scan-plans/:id | admin | Cron 定时计划编辑/删除 |
| 计划 | PATCH | /api/v1/scan-plans/:id/status | admin | 启停开关（启用/暂停） |
| 计划 | POST | /api/v1/scan-plans/batch-toggle | admin | 批量启用/禁用定时计划 |
| 任务 | GET/POST | /api/v1/tasks | admin/user | 任务列表/发起 |
| 任务 | GET | /api/v1/tasks/:id | 全部角色 | 任务详情（状态/进度/结果统计） |
| 任务 | POST | /api/v1/tasks/:id/stop | admin/user | 停止任务 |
| 任务 | POST | /api/v1/tasks/:id/rerun | admin/user | 失败重跑（复用原参数重新下发） |
| 任务 | DELETE | /api/v1/tasks/:id | admin | 删除历史任务 |
| 任务 | POST | /api/v1/tasks/batch-stop | admin/user | 批量停止任务 |
| 任务 | POST | /api/v1/tasks/batch-rerun | admin/user | 批量失败重跑 |
...
| 事件 | POST | /api/v1/events/:id/status | admin/user | 事件状态流转 |
| 事件 | POST | /api/v1/events/batch | admin/user | 批量状态流转（确认/关闭/归档） |
| 事件 | GET/POST | /api/v1/noise-rules | admin | 降噪规则 |
| 事件 | PUT/DELETE | /api/v1/noise-rules/:id | admin | 降噪规则编辑/删除 |
| 告警 | GET | /api/v1/alerts | 全部角色 | 告警列表（筛选/分页） |
| 告警 | GET | /api/v1/alerts/:id | 全部角色 | 告警详情 |
| 告警 | PATCH | /api/v1/alerts/:id | admin/user | 告警处置（确认/关闭/静默） |
| 告警 | POST | /api/v1/alerts/batch | admin/user | 批量告警处置 |
...
| 漏洞 | POST | /api/v1/vulnerabilities/:id/ticket | admin/user | 生成工单 |
| 漏洞 | POST | /api/v1/vulnerabilities/:id/retest | admin/user | 申请复测 |
| 漏洞 | POST | /api/v1/vulnerabilities/:id/ignore | admin/user | 忽略漏洞 |
| 漏洞 | POST | /api/v1/vulnerabilities/batch-ticket | admin/user | 批量生成工单 |
| 漏洞 | POST | /api/v1/vulnerabilities/batch-retest | admin/user | 批量复测 |
| 漏洞 | POST | /api/v1/vulnerabilities/batch-ignore | admin/user | 批量忽略 |
| 工单 | GET/POST | /api/v1/tickets | admin/user | 工单列表/创建 |
| 工单 | GET | /api/v1/tickets/:id | 全部角色 | 工单详情 |
| 工单 | PUT | /api/v1/tickets/:id | admin/user | 工单状态/派发 |
...
| 证据 | POST | /api/v1/evidence/screenshots | admin/user | 截图上传 |
...
| 报告 | DELETE | /api/v1/reports/:id | admin | 删除报告 |
| 报告 | GET/POST | /api/v1/report-templates | admin | 报告模板 |
| 报告 | PUT/DELETE | /api/v1/report-templates/:id | admin | 报告模板编辑/删除 |
| 场景 | GET/POST | /api/v1/scenarios | admin | 扫描场景列表/创建（重保/护网/日常/自定义） |
| 场景 | PUT/DELETE | /api/v1/scenarios/:id | admin | 场景编辑/删除 |
| 场景 | POST | /api/v1/scenarios/:id/activate | admin | 激活场景（重保/护网启用告警升级/值守/战报） |
| 场景 | POST | /api/v1/scenarios/:id/deactivate | admin | 停用场景 |
| 值守 | GET | /api/v1/watch/shift | 全部角色 | 值守班次列表/当前值班人 |
| 值守 | POST | /api/v1/watch/handover | admin/user | 交接班记录（交接时间/交接人/未处置告警与事件清单） |
...
| 升级 | GET/POST | /api/v1/escalation-rules | admin | 告警升级规则（级别门槛/延迟时间/升级对象） |
| 升级 | PUT/DELETE | /api/v1/escalation-rules/:id | admin | 升级规则编辑/删除 |
| 成员 | GET/POST | /api/v1/members | admin | 成员列表/邀请 |
| 成员 | POST | /api/v1/members/batch-invite | admin | 批量邀请（邮箱数组） |
| 成员 | POST | /api/v1/members/batch-remove | admin | 批量移除成员 |
| 成员 | PUT | /api/v1/members/:id | admin | 修改角色 |
| 成员 | POST | /api/v1/members/:id/disable | admin | 禁用成员 |
| 成员 | POST | /api/v1/members/:id/enable | admin | 启用成员 |
| 成员 | DELETE | /api/v1/members/:id | admin | 移除成员 |
| Worker | GET | /api/v1/worker/nodes | admin | Worker 节点管理 |
| Worker | GET/POST | /api/v1/worker/nodes/:id/boot-token | admin | Bootstrap Token |
| Worker | DELETE | /api/v1/worker/nodes/:id | admin | 移除离线 Worker 节点 |
| Worker | POST | /api/v1/worker/register | Worker Token | Bootstrap Token 换长期凭证（一次性） |
| 通知 | GET/POST | /api/v1/notify-channels | admin | 通知渠道（钉钉/企微/飞书/SMTP 多渠道） |
| 通知 | PUT/DELETE | /api/v1/notify-channels/:id | admin | 通知渠道编辑/删除 |
| 通知 | POST | /api/v1/notify-channels/:id/test | admin | 按 id 测试发送 |
| 通知 | GET/PUT | /api/v1/notify-routes | admin | 通知路由规则（severity/event_type → 渠道映射 + 默认渠道） |
| 规则库 | GET | /api/v1/rules | admin | 规则库版本/内容 |
| 规则库 | PUT | /api/v1/rules | admin | 规则热更新 |
| 规则库 | GET/POST | /api/v1/rules/items | admin | 规则项列表/新建（POC/敏感词/特征库/敏感信息规则集单项） |
| 规则库 | PUT/DELETE | /api/v1/rules/items/:id | admin | 规则项编辑/删除 |
| 规则库 | GET/POST | /api/v1/rules/import | admin | 规则库导入（POC/敏感词/特征库，敏感信息规则集支持 HaENet Rules.yml YAML） |
| 规则库 | GET | /api/v1/rules/export | admin | 规则库导出 |
| 情报 | GET/PUT | /api/v1/intel-subscriptions | admin | 情报订阅配置（数据源开关） |
| 情报 | GET | /api/v1/intel | 全部角色 | 情报列表（来源/严重程度/关键字筛选分页） |
| 情报 | GET | /api/v1/intel/:id | 全部角色 | 情报详情（含受影响资产列表） |
| 白名单 | GET/PUT | /api/v1/scan-whitelist | admin | 扫描授权白名单（允许目标域名/IP/网段 + 内网段黑名单） |
| 审计 | GET | /api/v1/audit-logs | admin | 审计日志（只读，筛选 operator/action/resource_type/start/end + 分页） |
| Token | GET/POST | /api/v1/api-tokens | admin | API Token 管理 |
| Token | DELETE | /api/v1/api-tokens/:id | admin | 撤销 Token |
| Token | PATCH | /api/v1/api-tokens/:id/status | admin | 临时停用/恢复 Token |
| Webhook | GET/POST | /api/v1/webhooks | admin | Webhook 配置 |
| Webhook | PUT | /api/v1/webhooks/:id | admin | 编辑 Webhook |
| Webhook | DELETE | /api/v1/webhooks/:id | admin | 删除 Webhook |
| Webhook | POST | /api/v1/webhooks/:id/test | admin | 触发测试推送 |
| Webhook | POST | /api/v1/webhooks/:id/secret | admin | 重新生成签名密钥（HMAC-SHA256） |
...
| 搜索 | GET | /api/v1/search | 全部角色 | 全局搜索（Bleve 跨 assets/findings/events，q 关键字，分页） |
...
- **图表基座（R5.19-4）**：ECharts 统一配色（主色/辅助色/成功/警告/危险/信息）、阈值配色（高危红/中危橙/低危黄/正常绿）、角色 Tag 颜色规范（admin 紫/user 蓝）。
...
- **危险操作确认**：删除/批量删除/移除成员/撤销 Token 等危险操作一律二次确认弹窗（含后果说明）。
...
- 连接鉴权：`?token={jwt}` 或首个消息携带 token，握手校验 JWT。
- 通道隔离：连接建立后由 JWT 鉴权，`Broadcast` 仅推送至已鉴权连接，禁止未授权订阅/接收（后端强制校验，不依赖前端过滤）。
- 消息帧（JSON）：
...
erDiagram
    ...
    licenses {
        int id PK
        string machine_hash "机器特征哈希（SHA-256）"
        datetime not_before "授权生效时间"
        datetime not_after "授权到期时间"
        int max_assets
        int max_workers
        string customer "客户标识"
    ...
    escalation_rules {
        ...
        string escalate_to "升级对象 admin / 值班组"
        bool enabled
        int version "乐观锁版本，默认 1"
    }
    watch_shifts {
    ...
    scenarios {
        ...
        string description
        bool activated "当前是否激活，至多一个激活场景"
...
**users 约束**：`users.role` 取 `admin`/`user`；`admin` 拥有全部配置/管理写权限，`user` 拥有业务模块写权限但无配置类写权限。

**审计日志表（audit_logs）**：`id, user_id, username, action, resource_type, resource_id, before_value, after_value, ip, user_agent, created_at`。禁止 update/delete，仅可 insert/select。筛选查询参数为 `operator`（映射 `username` 列，操作人）、`action`（操作类型）、`resource_type`（资源类型）、`created_at` 时间范围（start/end），与 R5.13-11 一致。ip 与 user_agent 在请求中间件统一捕获写入，不依赖前端上报。审计覆盖范围：登录/登出、资产增删改与批量、任务发起/停止/删除、事件/告警/漏洞/工单处置、成员与权限变更、策略/计划/规则/白名单/通知渠道/通知路由/API Token/Webhook 等配置变更；读操作与 Worker 引擎回传不审计，批量操作逐条记录。action 取值统一为 `<resource>.<verb>`（如 `auth.login / auth.logout / auth.delete_account / asset.create / asset.update / asset.delete / task.start / task.stop / event.acknowledge / alert.silence / alert.escalate / vuln.ignore / ticket.assign / member.invite / member.remove / rule.update / channel.update / token.create / webhook.create / license.import / scenario.activate / scenario.deactivate / watch.handover`），resource_type 取资源名（asset/task/event/alert/vuln/ticket/member/policy/plan/rule/whitelist/channel/route/token/webhook/license/scenario/watch）。

**API Token 表（api_tokens）**：`id, name, token_hash, scopes(JSON), status(active/disabled), expires_at, last_used_at`。scopes 取值为 RBAC 权限码（`src/config/permissions.ts` 清单，如 `asset:read`、`event:write`、`evidence:upload`）的子集，创建时勾选；请求校验时接口所需权限码必须是 token scopes 子集，token 无对应 scope 返回 2101 `SCOPE_DENIED`。scopes 随创建固定，修改需撤销重建。`status=disabled` 临时停用（`PATCH /api/v1/api-tokens/:id/status`），校验时拒绝，恢复为 `active` 后继续生效；撤销走 `DELETE /api/v1/api-tokens/:id`。

**通知渠道表（notify_channels）**：`id, type(dingtalk/wecom/feishu/smtp), config(JSON), enabled`。config 中密钥/令牌类字段（webhook_secret、smtp_password 等）入库前经 **AES-256-GCM 加密**（主密钥 `CINSIGHT_CHANNEL_KEY` 环境注入，独立于 JWT Secret），接口返回时掩码脱敏（如 `sk-****abcd`）；编辑时不回显明文，留空表示保持原值。

**通知路由表（notify_routes）**：`id, name, rule(JSON，severity/event_type → channel_ids 映射), default_channel_id, enabled`。告警触发时按 severity/event_type 匹配路由，未命中走默认渠道；路由层同时应用渠道启用开关与风暴抑制（单资产每小时上限）。rule JSON 结构：`{"rules":[{"severity":"critical|high","event_type":"*|篡改|端口暴露","channel_ids":[1,2]}, ...]}`，匹配优先级：先精确 `event_type` 后 `severity`，`*` 通配兜底；rule 不命中时用 `default_channel_id`。

**降噪规则表（noise_rules）**：`id, type(whitelist_ip/ignore_type/agg_window/storm_limit), config(JSON), enabled`。

**扫描授权白名单表（scan_whitelists）**：`id, kind(domain/ip/cidr), value, remark, enabled`，`unique(kind, value)`。Worker 以规则 Hash 周期同步白名单，发起请求前校验目标命中白名单且非内网段，未命中直接拒绝并计数。

**告警处置语义**：`PATCH /alerts/:id` 三态——`acknowledged`（确认，纳入处置跟踪）、`closed`（关闭，`resolved_at` 写入）、`silenced`（静默：抑制该资产同类新告警的通知推送与重新告警，告警记录保留可查；经再次 `PATCH` 可恢复为 open）。静默针对单资产维度的持续抑制，区别于 `noise_rules` 的全局/类型级过滤：静默由用户在告警中心按需操作，降噪规则由 admin 配置。

**重保/护网（HW）专项（R5.20）**：
- **告警升级通道**：`escalation_rules`（`id, name, trigger(级别门槛+延迟时间，如 critical-30), escalate_to(admin/值班组), enabled, version`）配置升级规则。处于 `scenario=important/hw` 时，`critical/high` 告警超过延迟时间仍未确认 SHALL 逐级升级：升级目标写入告警 `extra.escalated` 并加急推送（通知路由 `*` 通配兜底），升级动作写审计日志（`alert.escalate`）并广播 WebSocket 值守通道。
- **值守模式**：`watch_shifts`（`id, start_time, end_time, on_duty, handover_note, status(active/ended), created_at`）记录班次与交接。值守期间前端提供全屏值守视图（实时事件流 + 告警处置 + 升级状态 + 战报入口）；交接班经 `POST /api/v1/watch/handover` 记录交接人与未处置告警/事件清单。
- **每日战报**：`daily_war_reports`（`id, report_date, summary, file_path, created_at`）由 `CronScheduler` 在重保/护网激活期间按日生成（每日按 `CINSIGHT_TIMEZONE` 时区 00:30 生成前一日数据快照：新增/处置中/已闭环高危事件与告警数、TOP 风险资产、升级记录摘要、值守轮次），可经报告中心导出并推送通知渠道；次日生成不回溯当日。
- **重点资产加强监控**：`assets.importance=high` 标记资产纳入重保范围，其 `critical/high` 事件自动创建告警并进入升级通道。
- **场景启停**：`admin` 经 `POST /api/v1/scenarios/{id}/activate|deactivate` 控制场景激活；激活状态持久化于 `scenarios.activated`（至多一个激活场景，激活新场景自动停用旧场景）；授权失效自动停用并停止相关定时计划。

**场景表（scenarios）**：`id, name, scenario_type(daily/important/hw/custom), description, activated, activated_at, deactivated_at, created_at`。场景为可管理实体（`GET/POST /api/v1/scenarios`、`PUT/DELETE /api/v1/scenarios/:id`），激活后：重保/护网（`scenario_type=important/hw`）启用告警升级通道、值守模式与每日战报；`daily`/`custom` 激活仅作为平台展示的状态标记，不触发专项能力。
...
**定时计划表（scan_plans）**：`id, name, policy_id FK, asset_group_name, cron_expr, subpage_cron_expr(子页面监控频率，可空=跟随主频率，策略 crawl_subpages=true 时生效，见 CronScheduler), timezone, time_window, status(enabled/paused), last_run_at`。Cron 按 `timezone`（默认 `CINSIGHT_TIMEZONE`）计算执行时间；`time_window`（JSON，如 `{"start":"02:00","end":"06:00"}`，可空）限定计划触发后的执行时间窗口，窗口外触发的任务顺延至窗口内或跳过（见 CronScheduler）；暂停/启用经 `PATCH /:id/status` 切换。

**情报条目表（intel_items）**：全局情报库，`id, source(cve/cnvd/cnnvd), intel_id（CVE/CNVD/CNNVD 编号）, title, severity, scope（影响范围描述）, tech_stack（受影响技术栈 JSON，供资产匹配）, published_at, updated_at`，`unique(source, intel_id)`。由情报同步任务从 CVE/CNVD/CNNVD 数据源拉取更新；`GET /api/v1/intel` 列表与 `GET /api/v1/intel/:id` 详情读取本表，"受影响资产数"由引擎按资产技术栈匹配计算（设计 L478/L479 端点）。

**情报订阅表（intel_subscriptions）**：`id, source(cve/cnvd/cnnvd), enabled, last_sync_at`，`unique(source)`。

**报告模板表（report_templates）**：`id, name, sections(JSON，执行摘要/漏洞详情/内容安全/可用性统计/整改建议)，period(daily/weekly/monthly/quarterly/yearly，可空), cron_expr, timezone, enabled, updated_at`。定时方式二选一：`period` 快捷周期（日报/周报/月报/季报/年报，服务端内置 Cron 映射，如 `daily`→`0 0 * * *`、`weekly`→`0 0 * * 1`、`monthly`→`0 0 1 * *`、`quarterly`→`0 0 1 */3 *`、`yearly`→`0 0 1 1 *`）或自定义 `cron_expr`，二者至少其一（同时配置以 `period` 为准）；`cron_expr` 可空：设置后该模板启用定时报告，由 Master `CronScheduler` 按 `period`/`cron_expr`+`timezone`（默认 `CINSIGHT_TIMEZONE`）周期性生成 `reports`，调度语义与扫描计划一致（授权失效跳过触发）。

**报告表（reports）**：`id, name, template_id FK, title, asset_ids(JSON), period(JSON 覆盖时间范围), format(pdf/excel/screenshots), status(pending/generating/completed/failed), file_path, snapshot, created_at`。生成异步执行，进度经 `GET /api/v1/reports/:id` 轮询。

**Webhook 配置表（webhooks）**：`id, name, url, secret_hash, events(JSON 订阅事件列表), enabled, last_status(success/failed), last_error, retry_count`。推送经 HMAC-SHA256 签名（`X-Webhook-Signature`），失败重试 3 次后 `last_status=failed` 落库标记。

**Worker 节点表（worker_nodes）**：`id, name, ip, version, status(online/offline/offline_removed), heartbeat_at, load, boot_token_hash, client_id, client_secret_hash`。token/secret 仅存 hash（boot_token_hash=SHA-256，client_secret_hash=bcrypt），不落明文。状态判定：心跳间隔超 3 倍（默认 15s，`CINSIGHT_WORKER_HEARTBEAT_MS`=5000）未上报自动置 `offline`；移除节点置 `offline_removed` 不计入配额。调度器仅向 `online` 节点分配任务。

**时序降级表（availability_points / trend_points）**：VictoriaMetrics 的内嵌替代。`availability_points(id, asset_id, engine, sampled_at, status_code, response_ms)`（`engine` 区分 http/dns/tcp/ping 四维探针，status_code 记 HTTP 状态码或 TCP/DNS/PING 结果码，页面快照经事件证据链关联）, `trend_points(id, metric, value, sampled_at)`。预留 `MetricsExporter` 接口以支持未来切换 VM。

**微信公众号资产表（wechat_assets）**：`id, app_name(公众号名), wechat_id(微信号), avatar_url, fans_count, intro(简介), verify_status(认证状态), article_count(文章数), status, created_at`。

**证据-截图关联（evidence_files）**：`id, evidence_id, kind(html/har/screenshot/req/resp), file_path, md5, sha256, size, mime_type, created_at, expires_at`，支持一条证据链多文件（Req/Resp/HTML/截图各一文件）。`expires_at` 默认 `created_at + 365 天`，超期文件由清理任务删除并回收空间。
...
| 表 | 索引 | 目的 |
|----|------|------|
| assets | `idx_assets_url`, `idx_assets_group`, `idx_assets_importance` | 资产筛选 |
| scan_tasks | `idx_tasks_status`, `idx_tasks_created` | 队列监控/状态流转 |
| findings | `idx_findings_severity`, `idx_findings_engine`, `idx_findings_status`, `idx_result_id`（唯一） | 漏洞筛选 / 结果回传幂等去重 |
| vulnerabilities | `idx_vuln_severity`, `idx_vuln_status`, `idx_vuln_asset`, `idx_vuln_cve` | 漏洞列表/资产关联/CVE 检索 |
| alerts | `idx_alerts_status`, `idx_alerts_type`, `idx_alerts_severity`, `idx_alerts_asset` | 告警筛选/处置 |
| events | `idx_events_status`, `idx_events_type`, `idx_events_severity` | 事件筛选 |
| evidence | `idx_evidence_md5` | 证据去重 |
| availability_points | `idx_avail_asset_ts`（asset_id, sampled_at） | 时序查询 |
| trend_points | `idx_trend_metric_sampled`（metric, sampled_at） | 趋势聚合 |
| api_tokens | `idx_token_name` | Token 查询 |
| intel_items | `idx_intel_source_sev`（source, severity）, `idx_intel_id`（唯一：source, intel_id） | 情报列表筛选/编号检索 |
| worker_nodes | `idx_worker_status` | 节点管理 |
| watch_shifts | `idx_shift_status`（status） | 值守班次列表/当前值班人 |
| daily_war_reports | `idx_war_report_date`（report_date） | 每日战报按日期查询 |
| scenarios | `idx_scenarios_activated`（activated） | 场景激活状态查询 |
| audit_logs | `idx_audit_created`, `idx_audit_user`, `idx_audit_action`（username/action, created_at） | 审计筛选/查询 |
...
- 种子数据：启动时自动写入 `admin` 默认账户（首次初始化密码随机生成并打印一次）、初始策略模板、初始 POC/敏感词/木马特征库、场景预设模板（`scenario=daily/important/hw/custom` 各一，含对应引擎开关与强度参数）。首次启动需完成 `admin` 引导初始化（可通过 `--init-super-admin` 或首启向导），未初始化前平台管理功能禁用。
...
│   ├── http.ts          # axios 封装（Bearer + 401 拦截 + 错误提示）
...
│   ├── admin.ts         # 成员/设置/授权/Token/Webhook
│   └── ws.ts            # WebSocket 封装（订阅/重连）
├── stores/              # Pinia
│   ├── auth.ts          # token/用户/role
│   ├── menu.ts          # 动态菜单与路由
│   ├── asset.ts         # 资产列表状态
...
- 注册全局指令 `v-permission`，值支持角色数组或权限码（`v-permission="['admin','user']"` / `v-permission="'asset:delete'"`）。
...
| store | 职责 | 关键状态 |
|-------|------|---------|
| auth | 登录态/角色 | token, user, role, isAdmin |
...
5. **漏洞聚合**：漏洞类引擎（漏洞扫描/DNS）按 `asset_id+engine+签名` 聚合并入 vulnerabilities：首见创建（status=open），重复仅更新 `last_seen_at`；`vulnerabilities.closed_at` 记录关闭时间。
6. **告警生成**：severity≥high 或命中告警类型（可用性宕机/端口暴露/篡改/情报预警等）生成 alerts（status=open，resolved_at 空），走通知路由推送（severity/event_type→渠道）。
7. **WebSocket 广播**：新 event/alert 广播至所有在线连接；导航角标自增。
...
**SOP 模板库**：内置按事件类型分组的应急响应 SOP 模板（WebShell/内容违规/篡改/可用性/情报预警等，含"隔离→溯源→加固→验证"步骤），事件确认时按 event_type 自动挂载到 `events.sop_attached`；admin 在系统设置维护自定义 SOP（属 `noise_rules` 之外的独立 `sop_templates` 配置，可存于规则库 JSON）。
...
1. **写操作权限不变量**：任何 write API 必经 RBAC 中间件，无权限角色一律 403；该约束由中间件全局注册保障，controller 无法绕过。
2. **授权门禁不变量**：所有业务 Repository 查询先经 `LicenseRequired` 中间件校验授权有效性，授权失效时统一返回 2400–2404，controller 无法绕过。
...
9. **角色隔离**：`admin` 拥有配置/管理写权限，`user` 拥有业务读写权限；角色写入 users.role 单列。
...
| 禁用用户访问 | 401/403 | 2003 `USER_DISABLED` | AuthMiddleware 每次请求校验 users.status |
| 登录连续失败触发锁定 | 423 | 2002 `ACCOUNT_LOCKED` | 连续失败 5 次锁定 15 分钟，返回友好提示 |
| 角色无权限写操作 | 403 | 2100 `FORBIDDEN` | 前端隐藏入口 + Toast |
| 授权缺失/无效/机器不匹配/未生效/过期 | 403 | 2400 `LICENSE_REQUIRED` / 2401 `LICENSE_INVALID` / 2402 `LICENSE_EXPIRED` / 2403 `LICENSE_MACHINE_MISMATCH` / 2404 `LICENSE_NOT_YET_ACTIVE` | LicenseRequired 中间件校验授权文件，无授权仅放行 /api/health 与 /api/v1/license/* |
| 资产/成员/Worker 超配额 | 429 | 4290 `ASSET_QUOTA_EXCEEDED` / 4292 `MEMBER_QUOTA_EXCEEDED` / 4291 `WORKER_QUOTA_EXCEEDED` | 前端提示已达授权配额上限 |
| 授权配额之外的上限 | 409 | 3002 `LIMIT_EXCEEDED` | 前端提示资源已达上限 |
| 创建/导入资产 URL 与已有资产重复 | 409 | 3000 `DUPLICATE_URL` | 前端表单/导入报告逐行标注"URL 已存在" |
...
|------|------|
| 0 | 成功 |
| 1000-1099 | 参数校验（`VALIDATION_FAILED` 1000、`INVALID_FORMAT` 1002） |
| 2000-2099 | 认证（`AUTH_FAILED` 2000、`TOKEN_EXPIRED` 2001、`ACCOUNT_LOCKED` 2002、`USER_DISABLED` 2003） |
| 2100-2199 | 授权（`FORBIDDEN` 2100、`SCOPE_DENIED` 2101） |
| 2400-2499 | 离线授权（`LICENSE_REQUIRED` 2400、`LICENSE_INVALID` 2401、`LICENSE_EXPIRED` 2402、`LICENSE_MACHINE_MISMATCH` 2403、`LICENSE_NOT_YET_ACTIVE` 2404） |
| 3000-3099 | 业务冲突（`DUPLICATE_URL` 3000、`TASK_STATE_CONFLICT` 3001、`LIMIT_EXCEEDED` 3002） |
...
| `CINSIGHT_SUPER_ADMIN_USER` | admin | 初始管理员用户名 |
| `CINSIGHT_SUPER_ADMIN_PASS` | 空 | 初始管理员密码（空则随机生成） |
| `CINSIGHT_LICENSE_PATH` | 空 | 离线授权文件路径（.lic） |
| `CINSIGHT_LICENSE_PUBLIC_KEY` | 内置 | 授权验签公钥（内置默认厂商公钥，可覆盖） |
| `CINSIGHT_SMTP_HOST` | 空 | 系统级 SMTP 服务器（验证码/邀请邮件发送，独立于通知渠道） |
...
| `CINSIGHT_LLM_BASE_URL` | 空 | LLM 内容分类服务地址（OpenAI 兼容，管理员配置，未配置或超时回退正则） |
| `CINSIGHT_LLM_API_KEY` | 空 | LLM 服务密钥（K8s Secret 注入，禁止硬编码） |
| `CINSIGHT_LLM_MODEL` | deepseek-chat | LLM 内容分类模型名 |
...
| `CINSIGHT_EVIDENCE_TTL_DAYS` | 365 | 证据文件保留天数（超期清理回收） |
...
- 统一结构化日志（logrus/slog），输出 JSON，字段含 `ts, level, user_id, trace_id, path, latency_ms, status`（与 R4.5-5 一致；MVP 阶段先落轻量 `request_id`，阶段 4 接入 OpenTelemetry 后升级为 `trace_id`）。
...
| 模块 | 覆盖点 |
|------|--------|
| RBAC 中间件 | 两角色 × 读写操作矩阵（表驱动，按「RBAC 权限矩阵与权限码」逐模块断言，无权限写操作 403） |
| 认证服务 | bcrypt 校验、JWT 签发、refresh 换发、jti 黑名单、锁定阈值 |
...
| 授权门禁 | 授权缺失/无效/过期分别返回 2400/2401/2402、资产/成员/Worker 超限分别返回 4290/4292/4291、批量逐条 failed、到期写操作拒绝 |
...
- SQLite 内存模式 + BadgerDB 临时目录跑通 Repository 层授权门禁与查询。
...
- WebSocket 越权订阅拒绝（无订阅权限连接被拒）。

### 前端测试（vitest）

- 动态路由按 role 渲染权限（admin/user 两套菜单）。
- 证据抽屉 Hash 校验失败时标红提示。
- 万级列表虚拟滚动性能（TinyVue `virtual-scroll`）。
- 登录表单校验（字段格式/密码策略/错误提示）。
- 路由守卫（无 token 跳登录、越权角色 403）。
- 权限菜单渲染（按 RBAC 权限表过滤）。
...
- 成员管理/系统设置/平台管理/API Token/Webhook/审计日志。
...
- **CSRF 防护**：前端不依赖 Cookie 鉴权（Bearer 头），配合 `SameSite` Cookie 策略，无 CSRF Token 盲区。
- **API 通用限流**：网关层每用户/IP 速率限制（如 100 req/min），超限返回 HTTP 429 + `Retry-After` 头（秒）；登录接口单独限流（5 次/min/IP）+ 账户锁定（连续失败 5 次锁定 15 分钟）。
- **refresh token 机制**：登录签发短时效 access token（15min）+ 长时效 refresh token（7d）；`POST /api/v1/auth/refresh` 换发新 access token；登出、改密、密码重置后 SHALL 立即失效全部 refresh token（服务端黑名单，JWT jti 拉黑）。
...
- **禁用用户拦截**：AuthMiddleware 在每次请求校验用户 `users.status`，为 `disabled` 立即返回 401 `USER_DISABLED`；登录时同样拦截（403），已签发 JWT 在下次校验时失效。

### 授权配额限制（核心商业逻辑）

- **配额字段**：`max_assets` + `max_workers` 由离线授权文件导入时注入（对应授权记录 `licenses.max_assets/max_workers`），重新导入即覆盖并实时生效；`max_members` 成员配额为待实现项（授权文件载荷后续扩展）；有效期字段为 `not_before`/`not_after`。
- **校验时机（写入即校验，乐观计数）**：
  - 创建资产：已用资产数 ≥ `max_assets` → 4290 `ASSET_QUOTA_EXCEEDED`。
  - 邀请成员 / 批量邀请：成员数 ≥ `max_members` → 4292 `MEMBER_QUOTA_EXCEEDED`（待实现：授权文件载荷暂未携带 `max_members`）。
...
- **到期/失效行为**：授权 `not_after` 过期或文件缺失/校验失败时，Master 停止其 cron 计划、拒绝新建任务与资产写操作，仅保留 `/api/health` 与授权导入/查询接口；到期前 7 天开始每日提示（通知渠道 + 站内横幅）。
- **配额统计口径**：资产数以 `assets.status != 'deleted'` 计数；Worker 数以 `worker_nodes.status != 'offline_removed'` 计数；成员数以 `users.status == 'active'` 计数。`used_assets / used_workers / used_members` 为待实现项，落地后由 `GET /api/v1/license/status` 返回（当前仅返回 `status/days_remaining/not_before/not_after/max_assets/max_workers`）。
...
- **数据保留与归档**：事件/漏洞/告警热数据 180 天后冷归档（gzip + 独立卷）；审计日志保留 ≥ 365 天；提供清理任务与归档策略配置。归档数据保留只读查询能力：列表/详情端点接受 `archived=true` 参数路由到归档库（SQLite 附加库或独立卷只读打开），仅允许查询，写操作返回 403。
...
- **E2E 测试**：Playwright 覆盖登录→建资产→下发任务→证据抽屉→报告导出关键路径。
...
- **路由守卫**：`beforeEach` 校验 token 有效性、role 权限，非法跳转登录/403。
